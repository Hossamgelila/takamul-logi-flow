...
export default function Maintenance() {
  ...
  // Calculate analytics
  const totalMaintenanceCost = expenses.reduce((sum, exp) => sum + exp.amount_omr, 0);
  const avgMonthlyMaintenance = expenses.length > 0 ? totalMaintenanceCost / Math.max(1, getMonthsInPeriod()) : 0;
  
  // Group by vehicle
  const vehicleBreakdown = expenses.reduce((acc, exp) => {
  ...
  function getMonthsInPeriod(): number {
    switch (timeFilter) {
      ...
      case 'year': return 12;
      default: return Math.max(1, expenses.length > 0 ? 12 : 1); // Assume 12 months for 'all'
    }
  }
    ...
    <Layout>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Analyze maintenance costs from expense entries</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(avgMonthlyMaintenance)}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(expenses.length > 0 ? totalMaintenanceCost / expenses.length : 0)}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              {Object.entries(vehicleBreakdown)
                ...
                <div key={vehicle} className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{data.count} events</div>
                  </div>
              ...
              <TableBody>
                {expenses.slice(0, 10).map((expense) => (
                  <TableRow key={expense.id}>
                    ...
                    <TableCell>
                      {expense.truck_plate || expense.trailer_plate || 'Unassigned'}
                    </TableCell>
