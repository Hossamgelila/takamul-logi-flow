...
export default function Fleet() {
  ...
  useEffect(() => {
      ...
      .on(
        ...
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setTrucks(prev =>
              prev.filter(truck => truck.id !== payload.old.id)
            );
          }
        }
      )
      ...
      .on(
        ...
        payload => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setTrailers(prev =>
              prev.map(trailer =>
                trailer.id === payload.new.id
                  ? (payload.new as Trailer)
                  : trailer
              )
            );
          } else if (payload.eventType === 'DELETE') {
            setTrailers(prev =>
              prev.filter(trailer => trailer.id !== payload.old.id)
            );
          }
        }
      )
    ...
    console.log(
      '📡 Real-time subscriptions established for trucks and trailers'
    );
  ...
  const totalCapacityTrucks = trucks.reduce(
    (sum, truck) => sum + (truck.capacity_tons || 0),
    0
  );
  const totalCapacityTrailers = trailers.reduce(
    (sum, trailer) => sum + (trailer.capacity_tons || 0),
    0
  );
                  ...
                  <TableBody>
                    {filteredTrucks.map(truck => (
                      <TableRow key={truck.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={truck.active ? 'default' : 'destructive'}
                          >
                      ...
                      <TableRow key={trailer.id}>
                        ...
                        <TableCell>
                          {trailer.capacity_tons
                            ? `${trailer.capacity_tons} tons`
                            : '-'}
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant={
                              trailer.ownership === 'Owned'
                                ? 'default'
                                : 'secondary'
                            }
                          >
                        ...
                        <TableCell>
                          <Badge
                            variant={trailer.active ? 'default' : 'destructive'}
                          >
