    ...
    try {
      ...
      if (invoiceData) {
        ...
        setFormData({
          ...
          fxRateToOmr:
            parseFloat(invoiceData.fx_rate_to_omr?.toString() || '1') || 1,
        });

        // Populate items
        if (invoiceData.invoice_items) {
          const transformedItems: InvoiceItem[] = invoiceData.invoice_items.map(
        ...
        return {
          ...
          amount_omr:
            formData.currency === 'OMR'
              ? item.amount
              : item.amount * formData.fxRateToOmr,
        ...
        {loading && !originalInvoice ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Alert>
              ...
              <AlertDescription>
                Full Invoice Edit Mode: All fields, items, and settings can be
                modified. Changes will be saved immediately upon submission.
              </AlertDescription>
            </Alert>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setFormData({
                          ...formData,
                          fxRateToOmr: parseFloat(e.target.value) || 1,
                        })
                      }
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 space-y-2">
                  <Label htmlFor="tripMirrorReference">
                    Trip Mirror Reference
                  </Label>
                  <Input
                    ...
                    onChange={e =>
                      setFormData({
                        ...formData,
                        tripMirrorReference: e.target.value,
                      })
                    }
            ...
            <Card>
              ...
              <CardContent>
                ...
                {formData.truckId &&
                  ...
                  formData.trailerId !== 'none' && (
                    <div className="mt-4 space-y-2">
                      <Label htmlFor="availableTripMirror">
                        Available Trip Mirrors
                      </Label>
                      <Select
                        value={formData.tripMirrorReference}
                      ...
                      >
                        ...
                        <SelectContent className="z-50 border bg-background shadow-lg">
                          {availableTripMirrors.map(tripMirror => (
                            ...
                            >
                              {tripMirror.reference_number} -{' '}
                              {tripMirror.driver_name} (
                              {new Date(
                                tripMirror.start_date
                              ).toLocaleDateString()}
                              )
                            </SelectItem>
                          ))}
              ...
              <CardContent>
                <div className="space-y-4">
                  {items.map((item, index) => (
                    <div
                      key={item.id}
                      className="grid grid-cols-1 gap-2 rounded-lg border bg-card p-4 md:grid-cols-9"
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="h-8 rounded-md border bg-muted px-3 py-2 text-sm font-medium">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="h-8 rounded-md border bg-muted px-3 py-2 text-center text-sm">
                          {item.isPassThrough ? '✓' : '✗'}
                        </div>
                      </div>
                  ...
                  {items.length === 0 && (
                    <div className="rounded-lg border bg-muted/10 py-8 text-center text-muted-foreground">
                      ...
                      <p className="text-sm">
                        Click "Add Item" to get started with your invoice items.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-5">
                  <div className="rounded-lg bg-success/10 p-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {formData.currency !== 'OMR' &&
                        `≈ ${(totals.revenueSubtotal * formData.fxRateToOmr).toFixed(2)} OMR`}
                    </div>
                  </div>
                  <div className="rounded-lg bg-warning/10 p-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {formData.currency !== 'OMR' &&
                        `≈ ${(totals.passThroughSubtotal * formData.fxRateToOmr).toFixed(2)} OMR`}
                    </div>
                  </div>
                  <div className="rounded-lg bg-destructive/10 p-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {formData.currency !== 'OMR' &&
                        `≈ ${(totals.totalTax * formData.fxRateToOmr).toFixed(2)} OMR`}
                    </div>
                  </div>
                  <div className="rounded-lg bg-primary/10 p-3 md:col-span-2">
                    <span className="block font-medium text-muted-foreground">
                      Grand Total:
                    </span>
                    ...
                    <div className="text-sm font-medium text-muted-foreground">
                      {formData.currency !== 'OMR' &&
                        `≈ ${(totals.grandTotal * formData.fxRateToOmr).toFixed(2)} OMR`}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                    <div>
                      ...
                      <div className="font-medium">
                        {items.filter(i => !i.isPassThrough).length}
                      </div>
                    </div>
                    <div>
                      <span className="text-muted-foreground">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {formData.fxRateToOmr.toFixed(6)}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between border-t pt-4">
              <div className="text-sm text-muted-foreground">
                Last modified:{' '}
                {originalInvoice?.updated_at
                  ? new Date(originalInvoice.updated_at).toLocaleString()
                  : 'Never'}
              </div>
